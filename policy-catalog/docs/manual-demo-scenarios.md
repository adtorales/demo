...
| S5 schema evolution | Add `mb.company-policy.2.0.json` with a deliberately narrower enum. | `schema-compat.yml` rejects a non-major bump. With the major version, publish and verify the reconciler reports the existing non-compliant policy as `invalid` and leaves it frozen. |
